...
const Nav = () => {
  ...
  return (
      ...
      <div className="flex items-center gap-3">
        <img src={logo} alt="Logo" className={`w-14 h-14 ${dark && "bg-white rounded-full p-2"}`} />
      ...
      <div className="flex gap-5 xl:gap-20 items-center">
        ...
        <div className="space-x-6 flex items-center">
          {
            user ? <button onClick={handleLogout} className="btn hidden md:block text-lg px-5 xl:px-7">Logout</button>
              :
          ...
          <button onClick={() => setDark(!dark)} className='cursor-pointer'> <MdLightMode size={30} className={`${dark?"text-black":"text-white"}`} /> </button>
        </div>
      </div>

          
      <div
        className={`fixed top-0 left-0 h-full w-1/2 ${dark? "bg-gray-800 text-gray-200":"bg-white"} shadow-lg transform transition-transform duration-300 z-50 lg:hidden ${
         ...
         {links}
          {
            user ? <button onClick={handleLogout} className="btn md:hidden text-lg px-5 xl:px-7">Logout</button>
              :
              <Link to="/login"><button className="btn md:hidden text-lg px-5 xl:px-7">Login</button></Link>
                  }
          {
            !user && <Link to='/register'><button className="btn  md:hidden text-lg px-5 xl:px-7">Register</button></Link>
          }
        </div>
      </div>
